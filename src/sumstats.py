...
def _read_w_ld(args, log):
    ...
    col_names = ['SNP']
    if args.additive:
        col_names += ['LD_A_weights']
    if args.dominance:
        col_names += ['LD_D_weights']
...
def estimate_h2(args, log):
    ...
    for i in xrange(len(w_ld_cname)):
    ...
    if args.write_h2:
        ...
        h2_A = c * hsqhat_A.tot
        h2_D = c * hsqhat_D.tot
        intercept_A = hsqhat_A.intercept
        intercept_D = hsqhat_D.intercept

        # print .h2
        fout_h2 = open(args.out +'.h2','wb')
        print >>fout_h2, '\t'.join(map(str,[h2_A, intercept_A, h2_D, intercept_D]))
        fout_h2.close()
    ...
    # hsqhat = reg.Hsq(chisq_AC, transpose(ref_ld[:,1]),
    #              transpose(sumstats_AC[w_ld_cname[where_additive]]), transpose(sumstats_AC.N),
    #              M_annot[0,1].reshape((1,1)), n_blocks=n_blocks, intercept=intercept_h2,
    #              twostep=args.two_step, old_weights=old_weights)

    # log.log(hsqhat.summary(ref_ld_cnames, P=args.samp_prev, K=args.pop_prev))

    return hsqhat_A, hsqhat_D
