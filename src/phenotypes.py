	...
	for chr in xrange(args.n_chr):
		...
		if (((1 + int(args.dominance) + int(args.gxe)) * args.p_causal) < 1) or args.same_causal_sites: # If the number of runs through the data is less than 1, run this speedup.
			...
			if args.same_causal_sites is False:
				...
				if args.h2_E > 0: log.log('Picked {m} epistasis causal variants out of {mc}'.format(m=m_causal_E, mc=m_chr))
			...
			beta_A, beta_D, beta_E, beta_AC = [np.zeros(m_chr) for i in range(4)]
			...
			if args.epistasis:
				beta_E, beta_E_causal_index = np.zeros(m_chr), random.sample(xrange(m_chr), m_causal)
				log.log('Picked {m} epistasis causal variants out of {mc}'.format(m=m_causal, mc=m_chr))
				if args.h2_E > 0:
					beta_E[beta_E_causal_index] = np.random.normal(loc=0, scale=np.sqrt(args.h2_E / (m_total * args.p_causal)), size=m_causal)

			if args.gxe:
				beta_AC, beta_AC_causal_index = np.zeros(m_chr), random.sample(xrange(m_chr), mc=m_causal)
...
def get_chisq(args, tree_sequence_list_geno, m_geno, m_geno_total, y, N, C, log):
	...
	chisq_A, chisq_D, chisq_E, chisq_AC = [np.zeros((m_geno_total,1)) for i in range(4)]
	...
	if (((not args.case_control) or (args.case_control and args.linear)) and args.ldsc):
		if args.case_control:
	...
	if args.case_control:
		return chisq_A, chisq_D, chisq_E, chisq_AC, n, C_sim, index, y_cc, n_cases, T
	else:
		return chisq_A, chisq_D, chisq_E, chisq_AC, n, C_sim, index
